...
# Notify that we can run script only with root access and exit
...
for domain in ${domains[@]}; do
  # Use only results without www in this loop from array
...
docker-compose -f docker-compose-nginx.yml up -d nginx  && docker-compose restart nginx
...
for domain in ${!domains[*]}; do
  ...
  if [ -e "$data_path/conf/live/$domain_name/cert.pem" ]; then
    ...
    $email_arg --rsa-key-size $rsa_key_size --agree-tos --force-renewal --non-interactive" certbot
  fi
done
